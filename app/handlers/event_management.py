...
async def show_my_events(message: Message, session: AsyncSession, state: FSMContext):
    """Показать ТОЛЬКО опубликованные события пользователя"""
    ...
    if not events:
        await message.answer(
            "📅 <b>Ваши события</b>\n\n"
            "У вас пока нет опубликованных событий.\n"
            "Создайте первое событие кнопкой ниже!\n\n"
            "<i>События на модерации или отклонённые не отображаются</i>",
            reply_markup=get_main_keyboard(),
            parse_mode="HTML"
        )
        return
    ...
    await message.answer(
        f"📅 <b>Ваши опубликованные события</b>\n\n"
        f"Всего событий: {len(events)}\n\n"
        f"{events_text}\n\n"
        f"👇 <b>Выберите событие для управления:</b>\n\n"
        f"<i>События на модерации или отклонённые не отображаются</i>",
