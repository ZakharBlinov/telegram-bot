...
def upgrade_models():
    """Функция для добавления новых колонок в существующие таблицы"""
    import sqlite3
    ...
    try:
        cursor.execute("ALTER TABLE events ADD COLUMN status VARCHAR(20) DEFAULT 'pending'")
    except sqlite3.OperationalError:
        pass
    
    try:
        cursor.execute("ALTER TABLE events ADD COLUMN rejection_reason VARCHAR(500)")
    except sqlite3.OperationalError:
        pass
    
    try:
        cursor.execute("ALTER TABLE events ADD COLUMN moderated_by INTEGER")
    except sqlite3.OperationalError:
        pass
    
    try:
        cursor.execute("ALTER TABLE events ADD COLUMN moderated_at DATETIME")
    except sqlite3.OperationalError:
        pass
    
    try:
        cursor.execute("ALTER TABLE user_profiles_by_goal ADD COLUMN moderation_status VARCHAR(20) DEFAULT 'pending'")
    except sqlite3.OperationalError:
        pass
    
    try:
        cursor.execute("ALTER TABLE user_profiles_by_goal ADD COLUMN moderation_reason VARCHAR(500)")
    except sqlite3.OperationalError:
        pass
